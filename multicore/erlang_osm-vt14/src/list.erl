...
pmax(List, N, Death) when length(List) > N ->
    Lists = split(List, N),
    CollectPID = self(),
    Tmp = [{spawn_link(fun() -> worker(L, CollectPID, Death) end), L} || L <- Lists],
    Maxes = collect(length(Lists), [], Tmp),
    pmax(Maxes, N, Death);
pmax(List, _, _) ->
    list:max(List). 
...
collect(N, Maxes, Worker_PID) when length(Maxes) < N ->
    receive 
	{'EXIT', PID, random_death} ->
            %%collect(N, [-666|Maxes]);
            {_,X} = lists:keyfind(PID,1,Worker_PID),
            Pid_old = self(),
            collect(N,Maxes,[{spawn_link(fun() -> worker(X,Pid_old, death:start(60)) end),X}|Worker_PID]);
        {'EXIT', _PID, normal} ->
	    collect(N, Maxes,Worker_PID);
	Max -> 
	    collect(N, [Max|Maxes],Worker_PID) 
    end;

collect(_N, Maxes, _Worker_PID) ->
