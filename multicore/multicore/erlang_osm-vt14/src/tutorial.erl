...
simpsons(names) ->
    X = simpsons(),
    [Name || {Type, Gender, Name} <- X];
simpsons(males) ->
    X = simpsons(),
    [Name || {Type, Gender, Name} <- X, Gender == male];
simpsons(females) ->
    X = simpsons(),
    [Name || {Type, Gender, Name} <- X, Gender == female];
simpsons(pets) ->
    X = simpsons(),
    [Name || {Type, Gender, Name} <- X, Type /= person].
...
max([H | T]) ->
    F = fun(X, Y) when X < Y -> Y; (X, Y) -> X end,
    lists:foldl(F, H, T).
...
count(String, Char) ->
    F = fun(X, Y) when Char == X -> Y+1; (X, Y) -> Y end,
